...
const int kBlockSize = kNumLanes * sizeof(uint64_t);
const int kPacketSize = kGraphSize * kBlockSize;

class HighwayTwistedHashState512 {
 public:
  explicit INLINE HighwayTwistedHashState512(const uint64_t (&keys)[kNumLanes]) {
...
def x(a,b,c):
  ...
  INLINE void UpdatePacket(const uint64_t *packets) {
    V4x64U packet = LoadU(packets);
    Update(packet);
    packet = LoadU(packets + 1);
    Update(packet);
    packet = LoadU(packets + 2);
    Update(packet);
    packet = LoadU(packets + 3);
    Update(packet);
    packet = LoadU(packets + 4);
    Update(packet);
    packet = LoadU(packets + 5);
    Update(packet);
    packet = LoadU(packets + 6);
    Update(packet);
    packet = LoadU(packets + 7);
    Update(packet);
    packet = LoadU(packets + 4);
    Update(packet);
    packet = LoadU(packets + 1);
...
uint64_t HighwayTwistedHash512(const uint64_t (&key)[kNumLanes], const uint8_t* bytes,
                         const uint64_t size) {
  HighwayTwistedHashState512 state(key);
  ...
  for (int i = 0; i < truncated_size / sizeof(uint64_t); i += kPacketSize / sizeof(uint64_t)) {
